...
#     # Recebe as coordenadas de cada ponto
#     positions = {}
#     ncoordinates = coordinates[:]
#     del(ncoordinates[0:6])
#     del(ncoordinates[len(ncoordinates)-1])
#     for index, item in enumerate(ncoordinates):
#         ncoordinates[index] = item.split()
#     for k in ncoordinates:
#         positions["{}".format(k[0])] = k[1],k[2]
#     points = [i for i in range(1,len(positions)+1)]
...
#     distance = math.dist((int(pointaposition[0]), int(pointbposition[0])),(int(pointaposition[1]),int(pointbposition[1])))
#     return distance
def distanceTwoPoints(pontoa, pontob, dicionario):
    for i in dicionario:
        if i == pontoa:
...
#             alldistances["{}".format(k+j)] = distanceTwoPoints(str(k),str(j),dictionary)
#     return alldistances
def alldistances(points,dictionary):
    alldistances = {}
    points.append("R")
...
def generatenewpop(pop,positions):
    ...
    worst = evaluatenewpop[1]
    for element in evaluatenewpop:
        if element[1] > worst[1]:
            worst = element

    newpop.append(best[0])
    newpop.remove(worst[0])
    return evaluatenewpop, best

def geneticalgorithmwelitism(matrix):
    start = time.time()
